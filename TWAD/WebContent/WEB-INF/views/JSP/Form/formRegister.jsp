	...
	function validateForm() {
		...
		for (i = 0; i < inputVal.length; i++) {
			if (inputVal[i].val() == "") {
				...
				inputVal[i].focus();
				flag = false;
			} else if (inputVal[i].attr('id') == 'mobileNumId' && inputVal[i].val().length != 13) {
				inputVal[i].parent('div').find('label').after('<span class="error"> Please enter correct mobile No. </span>');
				inputVal[i].focus();
				flag = false;
